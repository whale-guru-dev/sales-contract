...
contract MoonFomo {
    ...
    uint256 public increment = 2000;
    uint256 public initialPrice = 10000000;
    ...
    function calcTicketCost(uint256 _amount) public view returns(uint256 sumCost) {
      ...
      /*
      uint256 a = (initialPrice * rounds[roundCount].ticketCount) + (rounds[roundCount].ticketCount * increment);
      uint256 b = (initialPrice * (rounds[roundCount].ticketCount + _amount)) + ((rounds[roundCount].ticketCount + _amount) * increment);

      sumCost = (a * _amount) + ((b - a) * _amount / 2);
      ...
      for(uint256 x = 0; x < _amount; x++){
        sumCost += initialPrice + holderPool.add(additionalPool).div(increment);
